import sys, os, time, json, csv, io, signal, warnings, shutil, traceback
...
def start_engine(service_obj, storage_path):
    ...
    try:
        ...
        service_obj.onStatusUpdate(f"Harvester RNS: {addr}")
    except Exception as e:
        service_obj.onStatusUpdate(f"Error: {str(e)}")
...
def send_report(target_hex, harvester_id, block_id, ripe, empty, lat, lon, photo_b64):
    try:
        # Create CSV exactly as the receiver expects
        ...
        csv_payload += f"{report_id},{harvester_id},{block_id},{ripe},{empty},{lat},{lon},{ts_str},{photo_b64}"
        ...
        lxm = LXMessage(target, local_destination, csv_payload, title="Harvest")
